...
```{r}
set.seed(100)
lstm_data1 <- as.matrix(Zip_Code_Results[, c(3,5,15,16,17,18,19,25)])
...
lstm.training <- lstm_data[ind == 1, 1:7]
lstm.test <- lstm_data[ind == 2, 1:7]
lstm.trainingtarget <- lstm_data[ind == 1, 8]
lstm.testtarget <- lstm_data[ind == 2, 8]
```
...
model %>% layer_lstm(units = 128,  input_shape = c(1,7), batch_size = 32, return_sequences = TRUE) %>%
...
```{r}
train_scores = model %>% evaluate(reshapeDt(lstm.training), lstm.trainingtarget, verbose = 0)
print(test_scores)
```


```{r}
test_scores = model %>% evaluate(reshapeDt(lstm.test), lstm.testtarget, verbose = 0)
print(test_scores)
```


```{r}
y_pred = model %>% predict(reshapeDt(lstm.training))
x_axes = seq(1:length(y_pred))
plot(x_axes, lstm.trainingtarget, type="l", col="red", lwd=2)
...
```{r}
y_pred = model %>% predict(reshapeDt(lstm.test))
x_axes = seq(1:length(y_pred))
plot(x_axes, lstm.testtarget, type="l", col="red", lwd=2)
lines(x_axes, y_pred, col="blue",lwd=2)
...
```{r}
pred_data <- rbind(lstm.training1, lstm.test1)
pred_data <- denormalizeData(pred_data, getNormParameters(lstm_data))
colnames(pred_data) <- colnames(lstm_data1)
